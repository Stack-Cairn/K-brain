...
func TestSynthTargetCommands(t *testing.T) {
	...
	writeCli(t, ext, `{"method":"k-brain.attached","params":{"tabId":42,"title":"X / chamath","url":"https://x.com/chamath"}}`)
	...
	for m, want := range map[string]string{
		...
		`{"id":3,"method":"Target.attachToTarget","params":{"targetId":"tab-42","flatten":true}}`: `"sessionId":"k-brain-ext"`,
	} {
		writeCli(t, cdp, m)
...
func TestNoTabAttachedErrors(t *testing.T) {
	...
	if resp := readSrv(t, cdp); !strings.Contains(resp, "click the k-brain extension icon") {
