...
func TestReplayUpdates(t *testing.T) {
	msgs := []ai.Message{
		...
		{Role: "assistant", Content: "looking", ToolCalls: []ai.ToolCall{toolCall("t1", "read", `{"path":"/f.go"}`)}},
		{Role: "tool", ToolCallID: "t1", Content: "1\tpackage f"},
		{Role: "assistant", Content: "", ToolCalls: []ai.ToolCall{toolCall("t2", "bash", `{"command":"false"}`)}},
		{Role: "tool", ToolCallID: "t2", Content: "Error: exit 1"},
		{Role: "tool", ToolCallID: "orphan", Content: "ignored"},
