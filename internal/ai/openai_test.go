...
func TestModels(t *testing.T) {
	...
	models, err := New(srv.URL+"/v1", "test-key").Models(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if len(models) != 2 {
...
func TestStreamRejectsNonSSEResponse(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, "<!doctype html><title>gateway</title>")
	}))
	defer srv.Close()
	_, _, err := New(srv.URL, "test-key").Stream(context.Background(), Request{Model: "m"}, nil, nil, nil)
	if err == nil || !strings.Contains(err.Error(), "expected SSE data") {
		t.Fatalf("expected non-SSE response error, got %v", err)
	}
}
...
func TestStripInternalLeavesCallerToolCallsIntact(t *testing.T) {
	tc := ToolCall{ID: "c1", Type: "function", DurationMs: 1234, ExitCode: 2}
	tc.Function.Name = "bash"
	history := []Message{{Role: "assistant", ToolCalls: []ToolCall{tc}}}
	out := stripAuthored(history)
	if got := out[0].ToolCalls[0]; got.DurationMs != 0 || got.ExitCode != 0 {
		t.Fatalf("wire copy should be stripped: %+v", got)
	}
	if got := history[0].ToolCalls[0]; got.DurationMs != 1234 || got.ExitCode != 2 {
