...
func TestParseClaudeInfersTypeAndMissingVar(t *testing.T) {
	os.Unsetenv("NO_SUCH_VAR_K_BRAIN_TEST")
	cfgs, err := ParseClaude([]byte(`{
		"mcpServers": {
			"a": {"command": "srv", "env": {"X": "$NO_SUCH_VAR_K_BRAIN_TEST"}},
	...
	if cfgs["a"].Env["X"] != "$NO_SUCH_VAR_K_BRAIN_TEST" {
...
func TestMergePrecedence(t *testing.T) {
	disabled := false
	kBrain := map[string]ServerConfig{"a": {Command: []string{"k-brain-a"}}, "b": {Enabled: &disabled, Command: []string{"k-brain-b"}}}
	...
	m := Merge(kBrain, codex, claude, global)
	if m["a"].Command[0] != "k-brain-a" {
		t.Error("k-brain config must win over codex and claude")
	}
	...
	if !m["b"].Disabled() {
		t.Error("k-brain-only entry should survive with enabled=false")
	}
}
...
func TestLoadMergedFilteredPolicy(t *testing.T) {
	...
	if f.Merged["node_repl"].Command[0] != "mine" {
		t.Error("k-brain config must still win over a blocked import")
	}
	if _, ok := f.Blocked["node_repl"]; ok {
		t.Error("no ghost row when k-brain owns the name")
	}
